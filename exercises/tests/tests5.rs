...
unsafe fn modify_by_address(address: usize) {
    // SAFETY: The caller must guarantee that `address` is a valid pointer
    // to a mutable `u32` value. It's the caller's responsibility to ensure
    // safety when using this function.
    unsafe {
        // The code block ensures that the provided `address` is used to modify
